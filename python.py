import json
tmp = open('whitelist.json')
df = json.load(tmp)
print(len(df))
for i in range(len(df)):
    tmp_mints = df[i]['whitelisted_mints']
    del df[i]['whitelisted_mints']
    df[i]['time_based_liquidity_pool'] = df[i]['liquidity_pool']
    df[i]['price_based_liquidity_pool'] = None
    df[i]['time_based_collection_info'] = df[i]['collection_info']
    df[i]['price_based_collection_info'] = None


    del df[i]['liquidity_pool']
    del df[i]['collection_info']
    df[i]['whitelisted_mints'] = tmp_mints

with open("moving_whitelist.json", "w") as file:
    json.dump(df, file, indent = 1)
    